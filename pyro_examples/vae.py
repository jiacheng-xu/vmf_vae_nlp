...
class VAE(nn.Module):
    ...
    def __init__(self, z_dim=50, hidden_dim=400, use_cuda=False):
        ...
        # create the enc and decoder networks
        self.encoder = Encoder(z_dim, hidden_dim)
        self.decoder = Decoder(z_dim, hidden_dim)

        if use_cuda:
            # calling cuda() here will put all the parameters of
            # the enc and decoder networks into gpu memory
            self.cuda()
    ...
    def guide(self, x):
        # register PyTorch module `enc` with Pyro
        pyro.module("enc", self.encoder)
        with pyro.iarange("data", x.size(0)):
            # use the enc to get the parameters used to define q(z|x)
