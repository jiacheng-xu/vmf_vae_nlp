...
def make_rand_vector(dims):
    vec = [np.random.uniform(-1, 1) for i in range(dims)]
...
for n in range(5):
    ...
    x = GVar(torch.FloatTensor(x).unsqueeze(0).expand(20,3))
    y = dis.sample_cell(x, None, 100)
    y = y.data.tolist()
    bag += y
...
fig.savefig('vmf.png', dpi=400,transparent=True)
plt.show()