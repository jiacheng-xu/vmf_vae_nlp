base = 'PYTHONPATH=../../ python ../nvll.py --cuda ' \
       '--lr 0.005 --batch_size 50 --eval_batch_size 50' \
       ' --log_interval 50 --model nvdm --epochs 80  ' \
       '--optim adam  --clip 2 ' \
...
bag = []
for drop in [0., 0.2]:
    for emsize in [100,400, 800]:
        for nhid in [100, 400, 800]:
            for aux in [0.1, 0.0001]:
                # for dist in ['unifvmf', 'vmf']:
                for dist in ['vmf']:
                    for kappa in [25,50,100,200,400]:
                        for lat_dim in [50]:
